...
@section('title', 'Tambah Role Access')
@section('page-title', 'Tambah Role Access')

@section('content')
<div class="mb-6">
    <h1 class="text-2xl lg:text-3xl text-black font-bold">Tambah Role Access</h1>
    <p class="text-gray-600 mt-1">Buat role access baru untuk sistem</p>
</div>

<div class="w-full lg:w-2/3 xl:w-1/2">
    <form class="p-6 lg:p-10 bg-white rounded shadow-xl" action="{{ route('admin.role-access.store') }}" method="POST">
        @csrf
        ...
        <div class="mb-4">
            ...
            <input class="w-full px-5 py-2 text-gray-700 bg-gray-200 rounded @error('name') border border-red-500 @enderror" 
                   id="name" name="name" type="text" required value="{{ old('name') }}" placeholder="Contoh: Kelola Balita">
            @error('name')
        ...
        <div class="mb-4">
            ...
            <input class="w-full px-5 py-2 text-gray-700 bg-gray-200 rounded @error('slug') border border-red-500 @enderror" 
                   id="slug" name="slug" type="text" value="{{ old('slug') }}" placeholder="Contoh: kelola-balita">
        ...
        <div class="mb-4">
            ...
            <textarea class="w-full px-5 py-2 text-gray-700 bg-gray-200 rounded @error('description') border border-red-500 @enderror" 
                      id="description" name="description" rows="3" placeholder="Deskripsi role access">{{ old('description') }}</textarea>
            @error('description')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <select class="w-full px-5 py-2 text-gray-700 bg-gray-200 rounded @error('permission') border border-red-500 @enderror" 
                ...
                @foreach($permissions as $permission)
                    <option value="{{ $permission }}" {{ old('permission') === $permission ? 'selected' : '' }}>
                        {{ ucfirst($permission) }}
                    </option>
                @endforeach
            </select>
