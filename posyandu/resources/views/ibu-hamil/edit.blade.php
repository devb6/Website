...
@section('page-title', 'Edit Data Ibu Hamil')

@section('content')
<h1 class="text-2xl lg:text-3xl text-black pb-4 lg:pb-6">Edit Data Ibu Hamil</h1>

<div class="w-full lg:w-2/3 xl:w-1/2">
    <form class="p-6 lg:p-10 bg-white rounded shadow-xl" action="{{ route('ibu-hamil.update', $ibuHamil) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <input class="w-full px-5 py-2 text-gray-700 bg-gray-200 rounded @error('nama') border border-red-500 @enderror" 
                   id="nama" name="nama" type="text" required value="{{ old('nama', $ibuHamil->nama) }}" placeholder="Nama Ibu Hamil">
            @error('nama')
        ...
        <div class="mb-4">
            ...
            <input class="w-full px-5 py-2 text-gray-700 bg-gray-200 rounded @error('nama_suami') border border-red-500 @enderror" 
                   id="nama_suami" name="nama_suami" type="text" value="{{ old('nama_suami', $ibuHamil->nama_suami) }}" placeholder="Nama Suami">
            @error('nama_suami')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <input class="w-full px-5 py-2 text-gray-700 bg-gray-200 rounded @error('tanggal_lahir') border border-red-500 @enderror" 
                   id="tanggal_lahir" name="tanggal_lahir" type="date" required value="{{ old('tanggal_lahir', $ibuHamil->tanggal_lahir->format('Y-m-d')) }}">
            @error('tanggal_lahir')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <input class="w-full px-5 py-2 text-gray-700 bg-gray-200 rounded @error('hpht') border border-red-500 @enderror" 
                   id="hpht" name="hpht" type="date" required value="{{ old('hpht', $ibuHamil->hpht->format('Y-m-d')) }}">
            @error('hpht')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <textarea class="w-full px-5 py-2 text-gray-700 bg-gray-200 rounded @error('alamat') border border-red-500 @enderror" 
                      id="alamat" name="alamat" rows="3" required placeholder="Alamat">{{ old('alamat', $ibuHamil->alamat) }}</textarea>
            @error('alamat')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <input class="w-full px-5 py-2 text-gray-700 bg-gray-200 rounded @error('telepon') border border-red-500 @enderror" 
                   id="telepon" name="telepon" type="text" value="{{ old('telepon', $ibuHamil->telepon) }}" placeholder="Nomor Telepon">
            @error('telepon')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
