...
@section('content')
<h1 class="text-2xl lg:text-3xl text-black pb-4 lg:pb-6">Tambah Data Balita</h1>

<div class="w-full lg:w-2/3 xl:w-1/2">
    <form class="p-6 lg:p-10 bg-white rounded shadow-xl" action="{{ route('balita.store') }}" method="POST">
        @csrf
        ...
        <div class="mb-4">
            ...
            <input class="w-full px-5 py-2 text-gray-700 bg-gray-200 rounded @error('nama') border border-red-500 @enderror" 
                   id="nama" name="nama" type="text" required value="{{ old('nama') }}" placeholder="Nama Balita">
            @error('nama')
        ...
        <div class="mb-4">
            ...
            <input class="w-full px-5 py-2 text-gray-700 bg-gray-200 rounded @error('nama_ibu') border border-red-500 @enderror" 
                   id="nama_ibu" name="nama_ibu" type="text" required value="{{ old('nama_ibu') }}" placeholder="Nama Ibu">
            @error('nama_ibu')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <input class="w-full px-5 py-2 text-gray-700 bg-gray-200 rounded @error('berat_lahir') border border-red-500 @enderror" 
                   id="berat_lahir" name="berat_lahir" type="number" step="0.01" value="{{ old('berat_lahir') }}" placeholder="Contoh: 3.5">
            @error('berat_lahir')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <input class="w-full px-5 py-2 text-gray-700 bg-gray-200 rounded @error('tinggi_lahir') border border-red-500 @enderror" 
                   id="tinggi_lahir" name="tinggi_lahir" type="number" step="0.01" value="{{ old('tinggi_lahir') }}" placeholder="Contoh: 50.5">
            @error('tinggi_lahir')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
